...
export default function VendorInventory() {
  ...
  // --- top-level page tabs ---
  type PageTab = "inventory" | "fulfillment";
  const [pageTab, setPageTab] = useState<PageTab>("inventory");

  // --- add / adjust (shared fields) ---
  ...
  // keep UI preview list in sync when open (quick client-side)
  useEffect(() => {
  ...
  async function handleSaveSingle() {
    ...
    await addInventoryEntry(vendorId, {
      ...
      note: note || undefined,
      shelf_life_days: normalizedLife,
    });
    setNote("");
    setShelfLifeSingle("");
    await load();
  ...
  async function handleCreateBulk(dryRun = false) {
    ...
    // Bulk uses the shelf life from the shared field above
    const normalizedLife =
      shelfLifeSingle.trim() === ""
        ? null
        : Math.max(0, Math.floor(Number(shelfLifeSingle)));
  ...
  async function onFulfilled(deliveryId: number) {
    try {
      await markDelivery("fulfill", deliveryId);
  ...
  return (
    <div className="mx-auto max-w-5xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-semibold">Inventory and Fulfillment</h1>
      ...
      {/* Top-level tabs: Inventory | Fulfillment */}
      <div className="flex gap-2 mb-4">
        <button
          className={`px-3 py-1.5 rounded-lg border text-sm ${pageTab === "inventory" ? "bg-base-200 border-base-300" : "border-transparent hover:bg-base-200"}`}
          onClick={() => setPageTab("inventory")}
        >
          Inventory
        </button>
        <button
          className={`px-3 py-1.5 rounded-lg border text-sm ${pageTab === "fulfillment" ? "bg-base-200 border-base-300" : "border-transparent hover:bg-base-200"}`}
          onClick={() => setPageTab("fulfillment")}
        >
          Fulfillment
        </button>
      </div>
      ...
      {pageTab === "inventory" ? (
        <>
          ...
          <div className="rounded-xl border border-base-300 bg-base-100 p-4 mb-4">
            ...
            {/* Shared fields (order: variant, quantity, shelf life, note) */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-3">
              {/* Select variant */}
              <select
                className="rounded-xl border border-base-300 bg-base-100 p-2 text-sm md:col-span-2"
                value={variantId ?? ""}
            ...
            <input
              ...
              className="w-full rounded-xl border border-base-300 bg-base-100 p-2 text-sm mb-3"
              value={note}
            ...
            <div className="flex gap-2 mb-3">
              <button
                className={`px-3 py-1.5 rounded-lg border text-sm ${
                  entryTab === "single" ? "bg-base-200 border-base-300" : "border-transparent hover:bg-base-200"
                }`}
              ...
              <button
                className={`px-3 py-1.5 rounded-lg border text-sm ${
                  entryTab === "bulk" ? "bg-base-200 border-base-300" : "border-transparent hover:bg-base-200"
                }`}
            ...
            {entryTab === "single" ? (
              ...
              <>
                ...
                <div className="flex gap-2 mt-3">
                  {/* Preview is purely client-side and should always be enabled */}
                  <button
                    className="btn"
                    onClick={() => {
                  ...
                  {/* Create entries uses backend bulk endpoint; requires variant */}
                  <button
                    className="btn btn-success"
                    onClick={() => handleCreateBulk(false)}
                    disabled={!variantId}
                    title={!variantId ? "Select a variant first" : undefined}
                  >
                    Create entries
                  </button>
                </div>
